...
run: async (client, message, args) => {
  ...
  connection.query("SELECT * FROM bot_cfg", async (err, result, fields) =>{
    ...
    let aberto = message.guild.channels.cache.find(c => c.name === `whitelist-${message.author.id}`) //buscar o canal pelo nome
    if (aberto){
      return message.channel.send(`Você já possui uma whitelist em aberta!`)
}
            if (err) throw err;
  ...
  // CONFIGURAÇÃO DA WHITE-LIST!
    ...
    if (err) throw err;
  if(message.channel.id !== result[0].canal_fazer_whitelist) return message.channel.send(message.author.toString() + ` Você não pode usar este comando nesse chat. Utilize: <#${result[0].canal_fazer_whitelist}>`)
...
for (const question of questions) {
  ...
  else if (reason == 'id') throw new Error('id')
...
createForm({ 
questions: [
"Qual seu nome e sua idade?", // respostas[0]

"Qual o ID apresentado no seu jogo?", // respostas[1]
...
.then(respostas => {
var exp = 0;
var is_a_number = respostas[1];
if(isNaN(is_a_number)){
  console.log(`ID Incorreto: ${is_a_number} - ${message.author.username}#${message.author.discriminator}`)
  channel2.delete();
  return message.author.send(`Você reprovou pois o ID [${is_a_number}] informado não é correto!`)
}

//   connection.query(`SELECT * FROM vrp_users WHERE id = '${is_a_number}' `, async (err, resultif, fields) =>{
// if(resultif === null || resultif === 0 || resultif[0].whitelisted === "1" || resultif[0].whitelisted === 1){
//   channel2.delete();
//   return message.author.send(`O ID [${is_a_number}] não existe/já está na whitelist. Tente novamente.`)
// }
...
//   })
  ...
  if(exp >= result[0].margemdeacertos){
    message.author.send(`Você foi **aprovado**, parabéns! Você teve ${exp}/${result[0].margemdeacertos} de acertos!`)
    channel2.delete()
  }
  else{
    message.author.send(`Você **reprovou** com ${exp}/${result[0].margemdeacertos} de acertos! Tente novamente.`)
    channel2.delete()
  }
  ...
  if(exp >= result[0].margemdeacertos){
      ...
      message.member.setNickname(nick).catch((err) => console.log("NAO CONSEGUI MUDAR O NOME DA PESSOA POIS ELE JÁ ERA DE UM GRUPO MAIOR QUE O MEU!"));        
  ...
  .setColor("#2b961f")//COR DA CAIXA DE DIALOGO
                    ...
                    .addField('PONTUAÇÃO:', `${exp}/${result[0].margemdeacertos}`)
                    .setAuthor('Real Life City', result[0].imgwl, 'https://www.reallifecity.com/')
  ...
  .setColor("#2b961f")//COR DA CAIXA DE DIALOGO
                    ...
                    .setAuthor('Whitelist - Real Life City', result[0].imgwl, 'https://www.reallifecity.com')
                    .setThumbnail(result[0].imgwl)
                    .setTimestamp(new Date())
                    .setFooter(`Parabéns! Agora você pode entrar no servidor.`)
                    client.channels.cache.get(result[0].resultadowl_acertos).send(embed2)
                    client.guilds.cache.get(result[0].iddoservidor).members.cache.get(message.author.id).roles.remove(result[0].nonwhitelistcargo).catch((err) => console.log("NAO CONSEGUI MUDAR O GROUP DA PESSOA POIS ELE JÁ ERA DE UM GRUPO MAIOR QUE O MEU!"))
                    client.guilds.cache.get(result[0].iddoservidor).members.cache.get(message.author.id).roles.add(result[0].whitelistcargo).catch((err) => console.log("NAO CONSEGUI MUDAR O GROUP DA PESSOA POIS ELE JÁ ERA DE UM GRUPO MAIOR QUE O MEU!"))
                    let tungsmember = client.guilds.cache.get(result[0].iddoservidor).members.cache.get(message.author.id);
                    let nickname = tungsmember ? tungsmember.displayName : null;
              
                    client.guilds.cache.get(result[0].iddoservidor).members.cache.get(message.author.id).setNickname('' + `${respostas[1]}` + ' | ' + nickname + '').catch((err) => console.log("NAO CONSEGUI RENOMEAR ESSA PESSOA!"))
...
console.log(`O usuário terminou o formulário e estas foram as respostas: ${respostas} com a pontuação de ${exp}/${result[0].margemdeacertos}.`)
})
...
if (err.message == 'time') {
  ...
  message.author.send(`Você reprovou pois o seu canal foi deletado. Tente novamente.`)
    ...
    console.log(`O usuário errou o ID.`)
    message.author.send(`Você errou o ID.`)
  }
 else {
