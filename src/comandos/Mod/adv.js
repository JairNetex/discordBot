...
message.delete({timeout:1000}).catch((err) => console.log("NAO CONSEGUI DELETAR UMA MENSAGEM POIS A MESMA JA FOI APAGADA!"));
if(!args[0]) return message.reply("Você não citou ninguém!").then(m => m.delete({timeout: 5000})).catch((err) => console.log("NAO CONSEGUI DELETAR UMA MENSAGEM POIS A MESMA JA FOI APAGADA!"));
...
message.reply("`" + advMember.user.tag + "` advertido com sucesso!").then(m => m.delete({timeout: 5000})).catch((err) => console.log("NAO CONSEGUI DELETAR UMA MENSAGEM POIS A MESMA JA FOI APAGADA!"));
